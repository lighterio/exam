...
var runBenchmark = module.exports = function (done) {
  ...
  var sampleSize = node.sampleSize || 100
  var runIndex
  ...
  function calculateStats () {
    ...
    if (passCount >= minimumSamples) {
      for (var i = children.length - 1; i > 0; i--) {
        ...
        // Get the slowest and fastest.
        var last = children[i]
        var best = children[0]

        // Calculate a maximum standard error by adding variances.
        var error = Math.sqrt((best.variance + last.variance) / passCount)

        // Calculate the Z value.
        var z = Math.abs(best.speed - last.speed) / error
