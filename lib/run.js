...
var options = process._EXAM_OPTIONS = JSON.parse(last);
...
/**
 * Optionally use an EcmaScript parser to preempt `SyntaxError` logging.
 */
if (options.parser) {
...
/**
 * Assign uncaught exceptions to the current node.
 */
process.on('uncaughtException', function (error) {
  currentNode.fail(error);
});

/**
 * Allow for the process to fail as the currentSuite after all nodes finish.
 */
...
/**
 * Move to the next suite or test.
 */
function nextNode() {
...
function Node(fn, name, isSuite) {
  var node = this;
  fn = fn || stub;
  node.fn = fn;
  node.name = name;
  node.isSuite = isSuite;
...
/**
 * Fail a test or suite due to an error.
 */
Node.prototype.fail = function (e) {
  var node = this;

  // If we must bail on the first error, set remaining nodes to be skipped.
  if (options.bail) {
    var node;
  ...
  // If it's our first time failing on this node, signal the reporter.
  if (!node.error) {
    reporter.fail();
  }
  ...
  // If the error is being added directly to the run, it's a parser error.
  if (node == run) {
...
Node.prototype.done = function (error) {
  var node = this;
...
Node.prototype.timeout = function (time, done) {
  var node = this;
  node.timeLimit = time;
  if (!node.isSuite) {
    clearTimeout(node.timer);
...
global.it = function (name, fn) {
  var node = new Node(fn, name, false);
