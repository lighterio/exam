...
is.equal = function (actual, expected) {
  var fn = (actual == expected) ? is.pass : is.fail;
  return fn(deep(actual) + ' == ' + deep(expected), is.equal, actual, expected, '==');
};

is.notEqual = function (actual, expected) {
  var fn = (actual != expected) ? is.pass : is.fail;
  return fn(deep(actual) + ' != ' + deep(expected), is.notEqual, actual, expected, '!=');
};
...
is.greater = function (first, second) {
  ...
  return fn(deep(first) + ' > ' + deep(second), is.greater, first, second, '>');
};

is.less = function (first, second) {
  var fn = (first < second) ? is.pass : is.fail;
  return fn(deep(first) + ' < ' + deep(expected), is.less, first, second, '<');
};

is.greaterOrEqual = function (first, second) {
  var fn = (first >= second) ? is.pass : is.fail;
  return fn(deep(first) + ' >= ' + deep(second), is.greaterOrEqual, first, second, '>=');
};

is.lessOrEqual = function (first, second) {
  var fn = (first <= second) ? is.pass : is.fail;
  return fn(deep(first) + ' <= ' + deep(second), is.lessOrEqual, first, second, '<=');
};
...
is.defined = is.notUndefined = function (value) {
  var fn = (typeof value != 'undefined') ? is.pass : is.fail;
...
is.instanceOf = function (value, expectedClass) {
  ...
  return fn(deep(value) + ' is an instance of ' + expectedClass.name, is.instanceOf, value, 'is an instance of');
};

is.notInstanceOf = function (value, expectedClass) {
  var fn = !(value instanceof expectedClass) ? is.pass : is.fail;
  return fn(deep(value) + ' is not an instance of ' + expectedClass.name, is.notInstanceOf, value, 'is not an instance of');
};
