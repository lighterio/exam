...
// When run by `exam`, `is` can continue assertions even if one fails.
var options = process._EXAM_OPTIONS;
var isContinuing = options ? !options.noContinuing : false;
...
// Allow `is` to track successful and failed assertions for each test.
var results = [];

// Rely on the `exam` runner to get results (which causes them to be cleared).
Object.defineProperty(is, 'getResults', {
  enumerable: false,
  value: function () {
    // If there's an error, return the results and reference a new array.
    if (results.error) {
      var value = results;
      results = [];
      return value;
    }
    // If the results are all positive, just clear them.
    else {
      results.length = 0;
    }
  }
...
is.fail = function (pieces, startFunction, actual, expected, operator) {
  ...
  // When in continuing mode, record the error and move on.
  if (isContinuing) {
    results.error = results.error || error;
    results.push(error);
  }
  // For other test runners, throw.
  else {
    throw error;
  }
  return is;
