...
module.exports = is.is = is.equal = is;
...
is.enableContinuation = function enableContinuation() {
  succeed = function succeed(statement) {
    // Try to find a calling function that's an "exam" test.
    var fn = arguments.callee.caller;
    while (fn.caller && !fn._EXAM_TEST) {
      fn = fn.caller;
    }
    ...
    if (stack.length > 10) {
      ...
      if (isArray) {
        data.forEach(function (value) {
          text = logger.deep(value, stack, indent);
        ...
        for (var key in data) {
          if (data.hasOwnProperty(key)) {
            ...
            text = key + colon + logger.deep(value, stack, indent);
