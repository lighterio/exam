...
var bench = require(libDir + '/bench')
...
var tree = module.exports = function (options) {
  options = options || require(libDir + '/options')()
  var grep = options.grep
  var ignore = options.ignore
  var reporter = require(libDir + '/reporters/' + options.reporter)
   ...
   */
  var is = scope.is = require(libDir + '/is')
  scope.mock = require(libDir + '/mock')
    ...
    while (true) {
      ...
      switch (node.phase) {
        ...
        case END:
          ...
          // TODO: Make a mode that streams results rather than dots.
          if (!isSuite) {
            if (showProgress) {
              reporter.pass()
            }
  ...
  // Create `only` and `skip` methods for `describe`, `it` and `bench`.
  var methods = [scope.describe, scope.it, scope.bench]
  ...
  // Create Jasmine-style shorthand functions.
  scope.ddescribe = scope.describe.only
  scope.xdescribe = scope.describe.skip
  scope.iit = scope.it.only
  scope.xit = scope.it.skip
  scope.dbench = scope.bench.only
  scope.xbench = scope.bench.skip

  // If not specifically told to run benchmarks, don't.
  if (!options.bench) {
    scope.bench = function () {}
    scope.bbench = scope.bench.only = scope.bench
    scope.xbench = scope.bench.skip = scope.bench
  }
  ...
  loggers.forEach(function (type) {
    scope[type] = function () {
      var now = Date.now()
      var stringify = reporter.stringify || stringify
      var args = []
